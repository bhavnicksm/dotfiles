...
# Bluetooth adapter power — modeled after Omarchy's omarchy-bluetooth-power.
# Single-purpose command, no UI: exit code is the interface.
#
# Usage:
#   bt-power.sh on|off|toggle|is-on

set -euo pipefail

POWER_WAIT_SECONDS=${BT_POWER_WAIT_SECONDS:-2}

usage() {
  echo "Usage: bt-power.sh <on|off|toggle|is-on>" >&2
  exit 1
}

# BlueZ never persists an adapter's Powered property, so turning Bluetooth off
# through bluetoothctl lasts only until the next boot. The rfkill soft block
# does persist: systemd-rfkill saves every switch under /var/lib/systemd/rfkill
# and restores it early on the next boot. So the block is the state, and BlueZ
# follows it: unblocking leaves AutoEnable at its stock default and bluetoothd
# powers the adapter up by itself. A plain `bluetoothctl power on` fails
...
power_on() {
  ...
  echo "bt-power: adapter did not come up" >&2
  return 1
}
